...
const API_KEY = "286fc5fa6c0330f0aef30ba81bc18cdf"
const API_Read_Access_Token = "eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyODZmYzVmYTZjMDMzMGYwYWVmMzBiYTgxYmMxOGNkZiIsInN1YiI6IjY0ZGY1NGUzYWFlYzcxMDNmN2UzNmQxMiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ._oVjlcjEZ3O6MCgP6an-XA2jZHFvi2bNr-wQuZfiTRs"
let isLetterClicked = false
let main = document.querySelector('.main')
let check = document.querySelector('.check')
let author = document.querySelector('.from')
let text = ' !"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`{|}~'
let letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
const code = getEncryption()
const encryptionKey = generateEncryptionKey()
const quoteObj = allQuotes[1]
// const quoteObj = allQuotes[Math.floor(Math.random()*allQuotes.length)]
author.textContent = "- " + quoteObj.from
const example = quoteObj.text
const sentence = example.toLocaleUpperCase().split(" ")
let lineWidth = 0
let newLine = document.createElement("div")
newLine.className = "newline"
sentence.forEach(word => {
    lineWidth += getWordWidth(word) + 3 + spaceWidth
    if(lineWidth > 600) {
        lineWidth = getWordWidth(word) + 3 + spaceWidth
        newLine = document.createElement("div")
        newLine.className = "newline"
        newLine.innerHTML += renderWord(word+" ").join("")
...
let entries = document.querySelectorAll('.container')
let allCharacters = document.querySelectorAll('.char-holder')

entries.forEach(entry => {
    entry.addEventListener("click", () => {
        let targetCode = entry.getAttribute("data-key")
        isLetterClicked = true
        if(JSON.parse(entry.getAttribute("data-active"))) {
            entries.forEach(en => {
                if(en.getAttribute("data-key") === targetCode) {
                    en.setAttribute("data-active", false)
                    en.style.background = "black"
                }
            })
        } else {
            entries.forEach(en => {
                ...
                en.style.background = "black"
                if(en.getAttribute("data-key") === targetCode) {
                    en.setAttribute("data-active", true)
                    en.style.background = "green"
                }
            })
        }
    })
...
window.addEventListener('keydown', e =>  {
    ...
    if(isLetterClicked && letters.split("").includes(e.key.toLocaleUpperCase())) {
        entries.forEach(entry => {
            if(JSON.parse(entry.getAttribute("data-active"))) {
               entry.textContent = e.key.toLocaleUpperCase()
            }
        })
    }
})
...
check.addEventListener("click", () => {
    allCharacters.forEach(entry => finalAnswer += entry.textContent)

    if (example.toLocaleUpperCase() === finalAnswer.trim()) {
        alert("You are correct!")
    } else {
        alert("Oops! that wasn't the correct answer, try again")
    }
})
...
function renderWord(word) {
    let quote = word.toLocaleUpperCase().split("").map(char => {
        if(char === " ") {
            return `<div class="char-holder space"> </div>`
        } else if(letters.includes(char)) {
            return `
            <div class="letter-container">
                <div data-solved=false data-active=false data-key=${encryptionKey[char]} class="char-holder container unsolved"></div>
                <div class="line"></div>
                <div class="encrypted-letter">${encryptionKey[char]}</div>
            </div> 
            `
        } else {
            return `<div class="char-holder punctuation">${char}</div>` 
        }
    })
...
function getEncryption() {
    let encryption = shuffleText(letters)
    while(encryption.split("").some((char, i) => char === letters[i])) {
        encryption = shuffleText(letters)
    }
